...
// Cek login & role anggota
if (!isset($_SESSION['logged_in']) || $_SESSION['logged_in'] !== true || $_SESSION['role_id'] != 2) {
    header("Location: ../../index.php?page=login");
    exit;
}

// Koneksi database (sudah ada di index_anggota.php, tapi untuk jaga-jaga kita require once)
// Pastikan path ini benar relatif dari file index_anggota.php yang meng-include file ini
...
<section id="articles">
  <div class="container articles-container">
    ...
    <div class="articles-grid">
      ...
      <?php
      // Query untuk mengambil 4 artikel terbaru dari database
      // Kita pakai LIMIT 4 agar tampilan home tidak terlalu penuh
      ...
      if ($result_artikel && $result_artikel->num_rows > 0) {
          while ($row = $result_artikel->fetch_assoc()) {
              // Path gambar artikel (sesuaikan jika berbeda)
              $gambar_path = "../../uploads/articles/" . $row['gambar'];
              // Jika gambar tidak ada di folder, pakai placeholder
              if (empty($row['gambar']) || !file_exists($gambar_path)) {
                  $gambar_path = "../../assets/img/aquara/ar1.png"; // Gambar default
              }

              // Buat cuplikan konten (hapus tag HTML dulu agar rapi)
    ...
    <a href="index_anggota.php?page=artikel_anggota" class="btn btn-more">
...
<?php include '../../includes/anggota/footer_home_anggota.php'; ?>