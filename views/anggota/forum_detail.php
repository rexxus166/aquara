<?php
// (Koneksi $conn sudah didapat dari index_anggota.php)
$activeMenu = 'forum';

// 1. Ambil ID Topik dari URL
$topic_id = $_GET['id'] ?? 0;
if ($topic_id == 0) {
...
if (isset($_GET['action']) && $_GET['action'] == 'delete_reply' && isset($_GET['reply_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href='index_anggota.php?page=forum_detail&id=$topic_id';</script>";
        exit;
    }
}

// --- HANDLER BARU: EDIT TOPIK (PLUS GAMBAR) ---
...
// 3. (PROSES FORM KOMENTAR) Tambah Komentar Baru
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['komentar'])) {
    ...
    if (!empty($komentar_text)) {
        ...
        if ($stmt_insert->execute()) {
            echo "<script>window.location.href='index_anggota.php?page=forum_detail&id={$topic_id}&status=success';</script>";
            exit;
        } else {
            ...
            <div class="post-header-flex">
                ...
                <div style="margin-left: auto; display: flex; align-items: center; gap: 10px;">
                    ...
                    <?php if ($topic['user_id'] == $user_id): ?>
                        ...
                        <a href="index_anggota.php?page=forum_detail&id=<?= $topic_id ?>&action=delete_topic" 
                        ...
                        <div class="reply-content-box">
                            <div class="reply-author">
                                ...
                                <?php if ($reply['user_id'] == $user_id): ?>
                                    <a href="index_anggota.php?page=forum_detail&id=<?= $topic_id ?>&action=delete_reply&reply_id=<?= $reply['id'] ?>" 
    ...
    if (shareBtn) {
        shareBtn.addEventListener('click', function() {
            ...
            if (navigator.share) {
                navigator.share({
                    title: 'AQUARA Forum',
                    text: 'Cek diskusi menarik ini: ' + this.dataset.judul,
                    url: url
                }).catch(console.error);
            } else {
