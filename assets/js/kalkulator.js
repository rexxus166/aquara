// kalkulator_pengunjung.js - Halaman kalkulator untuk pengunjung (belum login)

document.addEventListener('DOMContentLoaded', function() {
    
    // ========== FORM ELEMENTS - READ ONLY FOR GUEST ==========
    const formInputs = document.querySelectorAll('.kalkulator-form input, .kalkulator-form select');
    
    // Pastikan semua form input tidak bisa diisi oleh pengunjung
    formInputs.forEach(input => {
        input.addEventListener('click', function() {
            showLoginPrompt('mengisi form kalkulator');
        });
    ...
    window.showLoginPrompt = function(action) {
        ...
        let message = '';
        if (action === 'menghitung proyeksi') {
            message = 'Untuk menghitung proyeksi budidaya, Anda harus login atau register terlebih dahulu.';
        } else if (action === 'mengisi form kalkulator') {
            message = 'Untuk menggunakan kalkulator budidaya, Anda harus login atau register terlebih dahulu.';
        } else {
            message = `Untuk ${action}, Anda harus login atau register terlebih dahulu.`;
        }
        ...
        modal.innerHTML = `
            <div style="background: white; padding: 40px; border-radius: 20px; max-width: 500px; width: 90%; box-shadow: 0 10px 40px rgba(0,0,0,0.3); text-align: center; animation: modalFadeIn 0.3s ease;">
                ...
                <p style="color: var(--text-muted); margin-bottom: 20px; font-size: 14px;">
                    Dapatkan akses penuh ke kalkulator budidaya untuk menghitung estimasi biaya dan proyeksi keuntungan Anda.
                </p>
        ...
        showNotification('Silakan login untuk menggunakan kalkulator', 'warning');
    };
    ...
    // Tampilkan pesan bahwa ini adalah tampilan demo
    setTimeout(() => {
        const hasilPlaceholder = document.querySelector('.hasil-placeholder');
        if (hasilPlaceholder) {
            hasilPlaceholder.style.opacity = '0.7';
        }
    }, 500);
});